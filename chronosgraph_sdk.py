...
class ChronosGraphSDK:
    ...
    def get_context(self, agent_id: str, current_task: str, mode: str = "hybrid", max_episodes: int = 5) -> str:
        """High-level helper for formatted context with intelligent pruning."""
        context = "Relevant past experiences (including shared knowledge):\n"
        
        # 1. Semantic Recall
        # We limit the number of episodes to avoid context window overflow
        memories = self.recall(agent_id, current_task, context_type='semantic', limit=max_episodes)
        ...
        try:
            ...
            if relational:
                ...
                # Prune relational context to top 5 connections to avoid bloat
                for rel in relational[:5]:
    ...
    def summarize_memory(self, agent_id: str, older_than_n_episodes: int = 10) -> str:
        """
        Retrieves older episodes, summarizes them, and archives the original episodes.
        Returns the summary ID.
        """
        try:
            # 1. Get older episodes (skipping the most recent N)
            all_episodes = self.engine.get_episodes(agent_id, limit=100, include_archived=False)
            if len(all_episodes) <= older_than_n_episodes:
                logger.info(f"Not enough episodes to summarize for agent {agent_id}.")
                return ""

            # The engine returns episodes in DESC order (most recent first)
            # So older episodes are at the end of the list
            episodes_to_summarize = all_episodes[older_than_n_episodes:]
            
            # 2. Generate summary
            summary_text = self.summarizer.summarize_episodes(episodes_to_summarize)
            
            # 3. Store summary as a new 'summary' type episode
            summary_id = self.check_in(agent_id, {
                "content": f"SUMMARY of previous events: {summary_text}",
                "type": "summary"
            }, auto_extract=True)
            
            # 4. Archive old episodes
            episode_ids = [ep['episode_id'] for ep in episodes_to_summarize]
            self.engine.archive_episodes(agent_id, episode_ids)
            
            logger.info(f"Memory summarized for agent {agent_id}. Archived {len(episode_ids)} episodes.", extra={"agent_id": agent_id, "summary_id": summary_id})
            return summary_id
        except Exception as e:
