...
class TestMigrations(unittest.TestCase):
    ...
    def test_apply_migrations(self):
        """Test applying all migrations up to v4."""
        manager = MigrationManager(self.db_path)
        manager.apply_migrations()
        
        self.assertEqual(manager.get_current_version(), 4)
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Verify columns from v4 exist
            cursor.execute("PRAGMA table_info(episodes)")
            ep_columns = [row[1] for row in cursor.fetchall()]
            self.assertIn("is_archived", ep_columns)
            self.assertIn("metadata", ep_columns)
    ...
    def test_idempotent_migrations(self):
        ...
        self.assertEqual(version_after_first, 4)
