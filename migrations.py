...
class MigrationManager:
    ...
    def apply_migrations(self):
        ...
        migrations = [
            ...
            (4, self._migration_v4, "Episode archiving and summarization support"),
        ]
    ...
    def _migration_v4(self):
        """Episode archiving and summarization support (v4)."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            # Add is_archived column to episodes
            try:
                cursor.execute("ALTER TABLE episodes ADD COLUMN is_archived BOOLEAN DEFAULT 0")
            except sqlite3.OperationalError:
                pass
            
            # Add metadata column to episodes for summaries or other info
            try:
                cursor.execute("ALTER TABLE episodes ADD COLUMN metadata TEXT")
            except sqlite3.OperationalError:
                pass
